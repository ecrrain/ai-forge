...
if (-not $visionCandidates) {
    if ($hasGpu) {
        if ($vramGB -ge 8) {
            $list = @('qwen2.5vl:7b','llama3.2-vision:11b','qwen2.5vl:3b','minicpm-v','moondream')
        } elseif ($vramGB -ge 4) {
            $list = @('qwen2.5vl:3b','minicpm-v','moondream','smolvlm')
        } else {
...
if ($runtime -match 'ollama') {
    if ($vramGB -ge 8) { $ollamaTag = 'qwen2.5vl:7b' }
    elseif ($vramGB -ge 4) { $ollamaTag = 'qwen2.5vl:3b' }
